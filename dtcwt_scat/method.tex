...
% We carry this through to other chapters
...
% Note that the pseudocode can be for a simple routine or as part of an autograd
% block. The latter will be clear as it will involve saving of activations for the
% bacwkards pass.
% \begin{enumerate}
...
\Function{sfb1d}{$lo,\ hi,\ g_0,\ g_1,\ mode,\ axis$}
  ...
  \State $lo \gets \mathtt{pad}(lo,\ p,\ p,\ `zero')$ \Comment{pre pad the signal with zeros}
  \State $hi \gets \mathtt{pad}(hi,\ p,\ p,\ `zero')$ \Comment{pre pad the signal with zeros}
...
\begin{algorithmic}[1]
\Function{DWT.Forward}{$x,\ h_0^c,\ h_1^c,\ h_0^r,\ h_1^r,\ mode$}
...
% \begin{figure}
  ...
  % \mycaption{2-D $\DTCWT$ block diagram}{Two levels of the complex wavelet tree
  % for a real 2-D input image $x$. Image taken from \cite{kingsbury_image_1999}.}
