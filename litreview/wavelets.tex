...
rotations of this function by $a > 0,\ \bmu{b} \in \reals[2],\ \theta\in [0,
2\pi]$ where 
\begin{align}
...
these constraints are satisifed, we can recover $x$ from $CWT_x$ by:
\begin{equation}
...
is a commonly known frame where $A=B=1$ and $|\varphi_i|=1$ (e.g. the Discrete
...
Tight representations have other particular benefits \textbf{finish me}. 
...
\subsubsection{Deisgn Criteria for the $\DTCWT$}
  ...
  wavelet $\psiod_c = \psiod_r + j\psiod_i$ and complex father wavelet $\phiod_c =
  ...
  \begin{itemize}
    ...
    \item $g_0, g_1$ the low and high-pass analysis fitlers for $\phiod_i, \psiod_i$
  ...
  \begin{equation}
    g_0(n) \approx h_0(n-0.5) \rightarrow \psiod_g(t) \approx
    \mathcal{H}\{\psiod_h(t)\}
  \end{equation}
...
\subsubsection{2-D $\DTCWT$ and its Properties}
  ...
  $\psiod$ with $\phiod$ for both directions, and not taking the complex conjugate
