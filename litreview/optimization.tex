...
functions, a confusing but common practice. Note that $\sigma(x)$ and
$\F{Ber}(y; p)$ are defined as:
\begin{align}
  \sigma(x) &= \frac{1}{1+e^{-x}} \label{eq:ch2:sigmoid} \\
...
swapp the Bernoulli distribution for the Multinoulli or Categorical distribution,
and the sigmoid function for a softmax, defined by:
\begin{align}
  \sigma_i(x) &= \frac{e^{x_i}}{\sum_{k=1}^C e^{x_k}} \\
  \F{Cat}(y; \pi) &= \prod_{c=1}^C \pi_c^{\indic(y_c = 1)}
\end{align}
