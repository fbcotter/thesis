...
\end{equation}
  ...
  % \begin{figure}
    % \begin{center}
      ...
      % \input{\imgpath/scatternet_freqresp_plot1.tikz} 
  ...
  % may not be ideal as it loses a significant amount of information in doing
  % this. Instead, we can define scales $2^J$, over which the operator
...
% \subsection{Rotation and Translation Invariant Scatternets}
  ...
  % % \begin{figure}
    % % \centering
      ...
      % % \caption{Roto-Translation and Scale Invariant Scatternet block diagram. The
               ...
               % % scales. Taken from \cite{sifre_rotation_2013}.}
  ...
  % % \begin{figure}
    % % \centering
      ...
      % % \caption[Three dimensional convolution with roto-scale wavelet]
              ...
              % % wavelet. Image taken from \cite{sifre_rotation_2013}.}
      % % \label{fig:srs_3d}
  % % \end{figure}
