...
values, which could then be rectified by the ReLU. %
\begin{align}
...
desired sparsity level. We can do the latter by keeping a track of the moments
of the activations with an exponential moving average and finding quantiles by
assuming the distribution of the activations. For the bandpass coefficients,
the activations very nearly follow an exponential distribution. The lowpass
coefficients are a bit more varied, following a generalized Gaussian
distribution (GGD) with shape parameter $\beta$ ranging from 0.7 for the earlier
layers to 1.3 for the later layers.

\subsection{Non-Linearity Experiments}
Taking the same `gain1\_2\_3' architecture used for CIFAR-100, we combine the
action of the gain layer with the pixel nonlinearity into one bigger layer
dubbed a `WaveLayer', described in \autoref{alg:ch6:wavelayer}. In the wave
layer, we have 3 different nonliearities: the pixel nonliearity, the lowpass and
the bandpass.
...
Digesting this information gives us some useful insights. Firstly, soft and hard
thresholding the lowpass and bandpass coefficients does not perform 
%
\begin{algorithm}[t]
\caption{Nonlinear experiment `WaveLayer'}\label{alg:ch6:wavelayer}
\begin{algorithmic}[1]
...
\begin{table}[t]
  ...
  \begin{tabular}{l |p{\mywidth} p{\mywidth} p{\mywidth} p{\mywidth} p{\mywidth} }
      ...
      BN+MagReLU & \\
      $\mathcal{S}$\\
      $\mathcal{H}$ & 41.3 & 60.7 & & 52.0 & 40.6\\ 
      \bottomrule
    \end{tabular}
  }}\newline
  \subfloat[$\sigma_{pixel} = \F{ReLU}$]{
  \centering
  \makebox[\textwidth][c]{
  \begin{tabular}{l |p{\mywidth} p{\mywidth} p{\mywidth} p{\mywidth} p{\mywidth} }
      \toprule
