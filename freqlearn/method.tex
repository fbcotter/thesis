...
For a single subband in the $\DTCWT$. \textbf{FINISH ME}
...
\begin{figure}[t!]
  ...
  \mycaption{Block diagram of proposed DWT and $\DTCWT$ gain layers for $J=1$}{
  ...
  mixed independently with $C_{l+1}$ sets of convolutional filters.  After
  mixing, a possible wavelet based nonlinearity $\sigma_w$ is applied to the
  subbands, before returning to the pixel domain with an inverse $\DTCWT$.
  Figure~\autoref{fig:ch6:dwt_blk_diagram} shows the DWT gain layer, with only 3
  subbands and fewer learnt parameters, this is the simpler layer.
  Figure~\autoref{fig:ch6:dtcwt_blk_diagram} shows the $\DTCWT$ gain layer. With 6
  complex subbands and a larger lowpass, this is the more complex layer.}
