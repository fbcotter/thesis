...
On these networks we search over a range of learning rates, momenta, weight decay
...
\begin{table}[h]
  ...
  \begin{tabular}{c c}
    ...
    \midrule
    Learning Rate & $\left\{ 0.0316,\ 0.1,\ 0.316,\ 1 \right\}$ \\
    Momentum & $\left\{ 0,\ 0.25,\ 0.5,\ 0.9 \right\}$ \\
    Weight Decay & $\left\{ 10^{-5},\ 3.16\x 10^{-5},\ 10^{-4},\ 3.16\x 10^{-4} \right\} $\\
    Weight Gain & $\left\{0.5,\ 1.0,\ 1.5,\ 2.0 \right\}$
  \end{tabular}
\end{table}

\subsubsection{Initial MNIST Results and Proposed Expansions}\label{sec:ch5:mnist_initial}
\autoref{fig:ch5:mnist_comparison} shows histograms of the final accuracy for
the 256 runs from \autoref{tab:ch5:hyper_options}. These initial tests seem to indicate
that our proposed invariant layer is a poor substitute for a convolutional
